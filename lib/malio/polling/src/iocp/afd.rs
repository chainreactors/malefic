...
// NtCancelIoFileEx may be missing on older systems; declare it as link-time resolved to avoid runtime GetProcAddress failure.
...
fn is_pre_vista() -> bool {
    static PRE_VISTA: OnceLock<bool> = OnceLock::new();

    *PRE_VISTA.get_or_init(|| unsafe {
        let ntdll = match NtdllImports::get() {
            Ok(ntdll) => ntdll,
            Err(_) => return false,
        };

        let mut info: RtlOsVersionInfoW = std::mem::zeroed();
        info.dwOSVersionInfoSize = size_of::<RtlOsVersionInfoW>() as u32;

        let status = ntdll.RtlGetVersion(&mut info as *mut RtlOsVersionInfoW);
        status >= 0 && info.dwMajorVersion < 6
    })
}
        ...
        impl NtdllImports {
            unsafe fn load(ntdll: HMODULE) -> io::Result<Self> {
                ...
                let $name = {
                    ...
                    let addr = match addr {
                        Some(addr) => addr,
                        // NtCancelIoFileEx may be missing on some platforms (e.g. XP), fall back to the link-time compat shim.
                        None if NAME == concat!("NtCancelIoFileEx", "\0") => {
                            // Cast the function to the same type as GetProcAddress return value
                            transmute::<
                                unsafe extern "system" fn(HANDLE, *mut IO_STATUS_BLOCK, *mut IO_STATUS_BLOCK) -> NTSTATUS,
...
define_ntdll_import! {
    ...
    /// Cancels all ongoing I/O operations on a handle (available on pre-Vista systems).
    fn NtCancelIoFile(
        FileHandle: HANDLE,
        IoStatusBlock: *mut IO_STATUS_BLOCK
    ) -> NTSTATUS;
    ...
    /// Gets the OS version without compatibility shims/manifest issues.
    fn RtlGetVersion(
        VersionInformation: *mut RtlOsVersionInfoW
    ) -> NTSTATUS;
}
    ...
    T::Completion: AsIoStatusBlock + HasAfdInfo,
{
    ...
    pub(super) fn poll(
        ...
        // Pre-Vista (XP/2003) systems can get stuck with a never-completing AFD poll.
        // Use a finite relative timeout (in 100ns units) to force periodic completion.
        const PRE_VISTA_TIMEOUT_100NS: i64 = -10_000_000; // 1s
        ...
        let poll_info = unsafe {
            ...
            (*poll_info).timeout = if is_pre_vista() {
                PRE_VISTA_TIMEOUT_100NS
            } else {
                i64::MAX
            };
    ...
    pub(super) unsafe fn cancel(&self, packet: &T) -> io::Result<()> {
        ...
        let result = {
            ...
            if is_pre_vista() {
                // XP doesn't reliably support per-request cancellation; cancel all pending I/O on the AFD handle.
                ntdll.NtCancelIoFile(self.handle, cancel_iosb.as_mut_ptr())
            } else {
