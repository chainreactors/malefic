...
impl<T: CompletionHandle> IoCompletionPort<T> {
    ...
    pub(super) fn wait(
        ...
        #[cfg(target_arch = "x86")]
        {
            self.wait_xp_compat(packets, timeout)
        }

        #[cfg(target_arch = "x86_64")]
        {
            self.wait_vista_plus(packets, timeout)
        }
    }

    /// Windows Vista+ implementation using GetQueuedCompletionStatusEx
    #[cfg(target_arch = "x86_64")]
    fn wait_vista_plus(
        &self,
        packets: &mut Vec<OverlappedEntry<T>>,
    ...
    /// Windows XP compatible implementation using GetQueuedCompletionStatus
    #[cfg(target_arch = "x86")]
    fn wait_xp_compat(
        &self,
        packets: &mut Vec<OverlappedEntry<T>>,
        ...
        // XP compat: replace infinite timeout with short polling to avoid AFD driver bug causing permanent hang
        let timeout_ms = match timeout {
            None => 100,  // infinite timeout -> 100ms polling
            Some(d) => {
                let ms = dur2timeout(d);
                if ms == INFINITE {
                    100  // INFINITE -> 100ms
                } else {
                    ms.min(100)  // cap at 100ms
                }
            }
        };
        ...
        if result == 0 {
            let io_error = io::Error::last_os_error();
            let err_code = io_error.raw_os_error().unwrap_or(0);

            // Check if we got a completion packet despite the error
            if !overlapped_ptr.is_null() {
                // We have a packet, add it
            ...
            // XP compat: treat error code 0 or WAIT_TIMEOUT (258) as timeout
            if io_error.kind() == io::ErrorKind::TimedOut || err_code == 0 || err_code == 258 {
                return Ok(0);
            }
            return Err(io_error);
        }

        // Success - we got a completion packet
        if !overlapped_ptr.is_null() {
            let entry = OVERLAPPED_ENTRY {
                lpCompletionKey: completion_key,
                lpOverlapped: overlapped_ptr,
