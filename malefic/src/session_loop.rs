...
const REQUEST_RESPONSE_KEEPALIVE_POLL_INTERVAL: Duration = Duration::from_millis(10);
...
pub trait SessionIo {
    ...
    fn supports_duplex_keepalive(&self) -> bool {
        true
    }
}
...
impl SessionIo for ConnectionRunner {
    ...
    fn supports_duplex_keepalive(&self) -> bool {
        ConnectionRunner::supports_duplex_keepalive(self)
    }
}
...
pub trait SessionStrategy<A: SessionActor, R: SessionIo> {
    ...
    async fn request_response_keepalive_iteration(
        &mut self,
        actor: &mut A,
        runner: &mut R,
    ) -> anyhow::Result<bool>
    where
        ...
        R: Send,
    {
        self.heartbeat_iteration(actor, runner).await?;
        Ok(true)
    }
    ...
    R: SessionIo + Send,
{
    ...
    async fn request_response_keepalive_iteration(
        ...
        let has_outbound = !spites.spites.is_empty();
        let request = if has_outbound {
            spites
        } else {
            actor.create_ping()
        };
        ...
        let mut had_activity = has_outbound;
        if let Some(response) = runner.receive().await? {
            had_activity |= !response.spites.is_empty();
            actor.handle(response).await?;
        }
...
impl<S> SessionLoop<S> {
        ...
        S: SessionStrategy<A, R> + Send,
    {
        ...
        loop {
            ...
            if actor.keepalive_enabled() {
                match self
                    .run_keepalive_session(actor, runner)
                    .await
                    .map_err(classify_error)?
                {
    ...
    where
        ...
        S: SessionStrategy<A, R> + Send,
    {
        if !runner.supports_duplex_keepalive() {
            return self.run_request_response_keepalive(actor, runner).await;
        }
    ...
    async fn run_request_response_keepalive<A, R>(
        &mut self,
        actor: &mut A,
        runner: &mut R,
    ) -> anyhow::Result<LoopControl>
        ...
        S: SessionStrategy<A, R> + Send,
    {
        loop {
            let had_activity = self
                .strategy
                .request_response_keepalive_iteration(actor, runner)
                .await?;
            ...
            if !had_activity {
                Delay::new(REQUEST_RESPONSE_KEEPALIVE_POLL_INTERVAL).await;
            }
        }
    }
...
mod tests {
    ...
    struct FakeRunner {
        ...
        request_response_keepalive: bool,
    }
    ...
    impl SessionIo for FakeRunner {
        ...
        fn supports_duplex_keepalive(&self) -> bool {
            !self.request_response_keepalive
        }
    }
    ...
    #[test]
    fn request_response_keepalive_polls_without_duplex_upgrade() {
        let mut actor = FakeActor {
            keepalive_enabled: true,
            prepare_request_queue: VecDeque::from([empty_spites("poll")]),
            reconnect_on_handle_name: Some("done"),
            ..Default::default()
        };
        let mut runner = FakeRunner {
            request_response_keepalive: true,
            receive_queue: VecDeque::from([
                Some(keepalive_spites(false)),
                Some(empty_spites("done")),
            ]),
            ..Default::default()
        };
        let mut session_loop = SessionLoop::new(BeaconStrategy);
        ...
        assert_eq!(runner.downgrade_count, 0);
        assert_eq!(runner.log, vec!["send", "receive", "send", "receive"]);
        assert_eq!(
            actor.handled,
            vec!["keepalive".to_string(), "done".to_string()]
        );
    }
