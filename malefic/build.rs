...
fn link_yy_thunks(resources_dir: &PathBuf) {
    ...
    if obj_path.exists() {
        // YY-Thunks obj must be linked early so its __imp_WaitOnAddress etc.
        // override the std's dynamic GetProcAddress lookup.
        // Without /INCLUDE, MSVC linker discards the unreferenced obj and the
        // CRT$XID init hook never registers API fallbacks, causing crashes on Win7.
        let target_env = env::var("CARGO_CFG_TARGET_ENV").unwrap_or_default();
        if target_env == "msvc" {
            println!("cargo:rustc-link-arg=/INCLUDE:__YY_Thunks_Installed");
            println!("cargo:rustc-link-arg=/INCLUDE:__imp_WaitOnAddress");
            println!("cargo:rustc-link-arg={}", obj_path.display());
        } else {
            println!("cargo:rustc-link-arg=-Wl,--require-defined,__YY_Thunks_Installed");
            println!("cargo:rustc-link-arg={}", obj_path.display());
        }
        println!(
