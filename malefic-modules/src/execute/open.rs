use crate::prelude::*;
use malefic_process::exec;
use malefic_process::exec::is_file_in_use;
...
#[async_trait]
#[obfuscate]
impl malefic_module::ModuleImpl for Open {
    ...
    async fn run(
        ...
        if is_file_in_use(&request.path) && request.singleton {
            Ok(TaskResult::new_with_body(
                id,
                Body::ExecResponse(ExecResponse {
                    status_code: 0,
            ...
            Ok(TaskResult::new_with_body(
                id,
                Body::ExecResponse(ExecResponse::default()),
            ))
        }
    }
}
