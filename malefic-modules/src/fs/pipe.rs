...
impl malefic_module::ModuleImpl for PipeRead {
    async fn run(
        ...
        let has_active_server = {
            let servers = PIPE_SERVERS
                .lock()
                .map_err(|e| anyhow::anyhow!("PIPE_SERVERS lock poisoned: {}", e))?;
            servers.contains_key(&full_pipe_name)
        };

        if has_active_server {
            let cached_bytes = {
                let mut cache = PIPE_DATA_CACHE
                    .lock()
                    .map_err(|e| anyhow::anyhow!("PIPE_DATA_CACHE lock poisoned: {}", e))?;
...
impl PipeServer {
    fn pipe_servers(
    ) -> anyhow::Result<std::sync::MutexGuard<'static, HashMap<String, Arc<AtomicBool>>>> {
        PIPE_SERVERS
            .lock()
            .map_err(|e| anyhow::anyhow!("PIPE_SERVERS lock poisoned: {}", e))
    }
    ...
    fn pipe_data_cache(
    ) -> anyhow::Result<std::sync::MutexGuard<'static, HashMap<String, VecDeque<Vec<u8>>>>> {
        PIPE_DATA_CACHE
            .lock()
            .map_err(|e| anyhow::anyhow!("PIPE_DATA_CACHE lock poisoned: {}", e))
    }
    ...
    async fn start_server(
        ...
        {
            let mut servers = Self::pipe_servers()?;
            servers.insert(full_pipe_name.clone(), running.clone());
        }

        {
            let mut cache = Self::pipe_data_cache()?;
        ...
        let handle = std::thread::spawn(move || {
                ...
                loop {
                    ...
                    match pipe.read(&mut buffer) {
                        Ok(bytes_read) => {
                            ...
                            {
                                if let Ok(mut cache) = PIPE_DATA_CACHE.lock() {
            ...
            {
                if let Ok(mut servers) = PIPE_SERVERS.lock() {
    ...
    async fn get_server_status(&mut self, id: u32, pipe_name: &str) -> ModuleResult {
        ...
        let (is_running, cache_size) = {
            ...
            let cache = Self::pipe_data_cache()?;
