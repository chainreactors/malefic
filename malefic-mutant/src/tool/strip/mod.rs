...
pub fn strip_paths_from_binary(
    ...
    let mut patterns = vec![
        Regex::new(r"[A-Za-z]:\\[A-Za-z0-9_\\./\\-]*?\.rs").expect("Invalid Windows .rs path regex"),
        Regex::new(r"malefic[A-Za-z0-9_\\./\\-]*?\.rs").expect("Invalid malefic path regex"),
    ...
    for pattern in patterns {
        ...
        if !matches.is_empty() {
            ...
            for mat in matches.iter().rev() { // 从后往前替换，避免位置偏移问题
