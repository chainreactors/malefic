...
fn default_toolchain() -> String{ "nightly-2023-09-18".to_string()}
...
impl TargetConfig {
    ...
    pub fn validate_domain_suffix(&self) -> anyhow::Result<()> {
        if let Some(ref suffix) = self.domain_suffix {
            ...
            if suffix.parse::<std::net::IpAddr>().is_ok() {
                return Err(anyhow::anyhow!("domain_suffix cannot be an IP address: {}", suffix));
            }

            // 验证后缀是有效的域名格式
            if suffix.contains(':') {
                return Err(anyhow::anyhow!("domain_suffix should not contain port: {}", suffix));
            }
        }
        Ok(())
    }
    ...
    pub fn validate(&self) -> anyhow::Result<()> {
        ...
        if protocol_count > 1 {
            return Err(anyhow::anyhow!("Target can only have one protocol configuration"));
        }
...
impl BasicConfig {
    ...
    pub fn validate_targets(&self) -> anyhow::Result<()> {
        ...
        for (index, target) in self.targets.iter().enumerate() {
            target.validate().map_err(|e| {
                anyhow::anyhow!("Target {} validation failed: {}", index, e)
            })?;
        }
    ...
    fn validate_unique_addresses(&self) -> anyhow::Result<()> {
        ...
        for target in &self.targets {
            if !addresses.insert(&target.address) {
                return Err(anyhow::anyhow!("Duplicate address found: {}", target.address));
            }
        }
        Ok(())
    }
    ...
    pub fn has_tls_enabled(&self) -> bool {
        self.targets.iter().any(|t| {
            t.tls.as_ref().map_or(false, |tls| tls.enable)
        })
    }
    ...
    pub fn validate_dga_config(&self) -> anyhow::Result<()> {
        if self.dga.enable {
            if self.dga.key.is_empty() {
                return Err(anyhow::anyhow!("DGA key cannot be empty when DGA is enabled"));
            }

            if self.dga.interval_hours == 0 || self.dga.interval_hours > 24 {
                return Err(anyhow::anyhow!("DGA interval_hours must be between 1 and 24"));
            }
            ...
            println!("DGA enabled with {} domain suffixes: {:?}", domain_suffixes.len(), domain_suffixes);
        }
        Ok(())
    }
}
