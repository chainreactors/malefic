        ...
        Value::Tagged(tagged) => {
            if tagged.tag == Tag::new("!File") {
                if let Value::String(file_path) = &tagged.value {
                    ...
                    let file_content = fs::read(resource_filepath.clone())
                        .unwrap_or_else(|err| {
                        panic!("Failed to read file '{:?}': {} at {}", 
                            resource_filepath, err, path);
                    });
                    *value = Value::Sequence(
                        file_content.into_iter().map(|byte| {
                        Value::Number(Number::from(byte as i64))
                    }).collect());
                }
                ...
                if let Value::String(encoded_str) = &tagged.value {
                    let decoded_bytes = BASE64_STANDARD.decode(encoded_str)
                        .unwrap_or_else(|err| {
                        panic!("Failed to decode base64 string '{}': {} at {}",
                            encoded_str, err, path);
                    });
                    *value = Value::Sequence(decoded_bytes.into_iter().map(
                        |byte| {
                            Value::Number(Number::from(byte as i64))
                        }
                    ).collect());
                ...
                if let Value::String(hex_str) = &tagged.value {
                    let decoded_bytes = hex::decode(hex_str).unwrap_or_else(
                        |err| {
                            panic!("Failed to decode hex string '{}': {} at {}", 
                                hex_str, err, path);
                        }
                    );
                    *value = Value::Sequence(decoded_bytes.into_iter().map(
...
{
    let mut yaml_value: Value = from_str(yaml_str)
        .expect("Failed to parse YAML");
    process_tags(&mut yaml_value, "root");
    serde_yaml::from_value(yaml_value)
        .expect("Failed to deserialize into target type")
}
