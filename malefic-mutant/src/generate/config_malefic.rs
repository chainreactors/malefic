...
pub fn update_beacon_toml(server: &BasicConfig, implant_config: &ImplantConfig) {
    ...
    if let Some(features) = cargo_toml[&FEATURES].as_table_mut() {
        ...
        if has_pack || !implant_config.prelude.is_empty() {
            log_success!("prelude is enabled");
...
pub fn update_malefic_spites(implant_config: &ImplantConfig, key: &str) -> anyhow::Result<()> {
    ...
    let mut spites = if !implant_config.prelude.is_empty() {
        log_info!(
            "Loading autorun configuration from {}",
            implant_config.prelude
        );
        let autorun_yaml = std::fs::read_to_string(&implant_config.prelude)?;
