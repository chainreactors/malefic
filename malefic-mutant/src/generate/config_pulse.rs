...
use crate::{
    log_success, CFG_TARGET_OS_WINDOWS, CONFIG_MALEFIC_WIN_KIT_PATH, DEFAULT, DEPENDENCICES,
    FEATURES, MALEFIC_WIN_KIT, PATH, PREBUILD, SOURCE, TARGET,
};
...
pub fn update_pulse_toml(source: bool) {
    ...
    let cargo_toml_content =
        fs::read_to_string(config_pulse_toml_path).expect("Failed to read Cargo.toml file");
    ...
    if let Some(features) = cargo_toml[&FEATURES].as_table_mut() {
        if let Some(default_array) = features[&DEFAULT].as_array_mut() {
            if source {
                ...
                if default_array
                    .iter()
                    .find(|x| x.as_str().unwrap() == PREBUILD)
                    .is_none()
                {
        ...
        if let Some(target_os) = target[&CFG_TARGET_OS_WINDOWS].as_table_mut() {
            if let Some(dependencies) = target_os[&DEPENDENCICES].as_table_mut() {
                if !source {
                    ...
                    inline_table.insert(PATH, Value::from(CONFIG_MALEFIC_WIN_KIT_PATH));
                    inline_table.insert(DEFAULT_FEATURE, Value::from(false));
                    let mut array = Array::default();
                    array.push(NANO_FEATURE);
                    array.push(ASM_FEATURE);
                    inline_table.insert(FEATURES, Value::from(array));
                    dependencies.insert(&MALEFIC_WIN_KIT, Item::Value(inline_table.into()));
                }
            }
        }
    }
