...
fn plain_str_expr(value: &str) -> String {
    format!("{:?}", value)
}
...
fn generate_runtime_config_inline_expr(server: &BasicConfig, use_obfstr: bool) -> String {
    ...
    format!(
        ...
        retry = server.retry,
        max_failures = server.max_failures.unwrap_or(10),
...
fn generate_transport_config(target: &TargetConfig, use_obfstr: bool) -> String {
    if let Some(http_config) = &target.http {
        ...
        // Resolve Host header with 3-tier priority:
        // 1. headers["Host"] (user-specified in headers map)
        // 2. http.host (dedicated host field, for domain fronting)
        // 3. target.address (fallback)
        let host_value = http_config
            .headers
            .get("Host")
            .filter(|v| !v.is_empty())
            .cloned()
            .unwrap_or_else(|| {
                if !http_config.host.is_empty() {
        ...
        headers_code.push_str(&format!(
            ...
            obf_or_plain_expr(&host_value, use_obfstr)
        ));

        // Generate other headers (skip Host since it's handled above)
...
"#,
            method = plain_str_expr(&http_config.method),
            path = plain_str_expr(&http_config.path),
            version = plain_str_expr(&http_config.version),
...
mod tests {
    ...
    #[test]
    fn test_plain_str_expr_emits_borrowed_literal() {
        let result = plain_str_expr("POST");
        assert_eq!(result, r#""POST""#);
        assert!(
            !result.contains(".to_string()"),
            "plain_str_expr must emit &str-compatible literals"
        );
    }
