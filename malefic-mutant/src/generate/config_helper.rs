...
pub fn update_helper_toml(version: &Version, source: bool) {
    ...
    let cargo_toml_content =
        fs::read_to_string(config_helper_toml_path).expect("Failed to read Cargo.toml file");
        ...
        if let Some(default_array) = features[&DEFAULT].as_array_mut() {
            ...
            match version {
                Version::Community => {
                    if default_array
                        .iter()
                        .find(|x| x.as_str().unwrap() == COMMUNITY)
                        .is_none()
                    {
                        default_array.push(COMMUNITY);
                    }
                    default_array.retain(|x| x.as_str().unwrap() != PROFESSIONAL);
                }
                Version::Professional => {
                    if default_array
                        .iter()
                        .find(|x| x.as_str().unwrap() == PROFESSIONAL)
                        .is_none()
                    {
        ...
        if let Some(target_os) = target[&CFG_TARGET_OS_WINDOWS].as_table_mut() {
            if let Some(dependencies) = target_os[&DEPENDENCICES].as_table_mut() {
                if !source {
                    ...
                    inline_table.insert(PATH.to_string(), Value::from(CONFIG_MALEFIC_WIN_KIT_PATH));
