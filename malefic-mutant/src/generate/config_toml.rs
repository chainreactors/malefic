...
pub fn update_config_toml() {
    ...
    if config_path.exists() {
        existing_content =
            fs::read_to_string(config_path).expect("Failed to read config.toml file");
    }
    ...
    let cfg_any_key =
        "cfg(any(target_family = \"windows\", target_family = \"unix\", target_family=\"wasm\"))";

    if !doc[target_key]
        .as_table_mut()
        .unwrap()
        .contains_key(cfg_any_key)
    {
    ...
    if let Ok(mut file) = fs::File::create(config_path) {
        if let Err(e) = file.write_all(doc.to_string().as_bytes()) {
            log_error!("Failed to write updated content to config.toml: {:?}", e);
        } else {
            log_success!(
                "config.toml file at \".cargo/config.toml\" has been updated successfully"
            );
        }
    } else {
        log_error!("Failed to create config.toml file at \".cargo/config.toml\"");
    }
