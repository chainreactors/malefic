...
pub fn module_impl(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let rt_module_impl = quote! {
        ...
        const _: () = {
            impl ::malefic_module::rtmodule::RtModule for #struct_ty {
                fn name() -> &'static str {
                    ::malefic_gateway::obfstr!(#module_name)
                }
                fn new() -> Self {
                ...
                fn run(
                    &mut self,
                    task_id: u32,
                    channel: &::malefic_module::rtmodule::RtChannel,
                ) -> ::malefic_module::rtmodule::RtResult {
                    use ::malefic_module::rtmodule::RtResult;
