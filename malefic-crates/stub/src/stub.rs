...
pub fn build_connection_for_server(
    ...
    malefic_transport::ConnectionBuilder::new(transport)
        ...
        .with_duplex_keepalive(supports_duplex_keepalive_for_server(server_config))
        .build()
        .map_err(|e| anyhow::anyhow!("Failed to build connection: {:?}", e))
}

pub fn supports_duplex_keepalive_for_server(server_config: &config::ServerConfig) -> bool {
    !matches!(
        server_config.transport_config,
        config::TransportConfig::Http(_)
    )
}
...
pub fn session_config_for_server(
    ...
    let is_request_response = matches!(
        server_config.transport_config,
        config::TransportConfig::Http(_)
    );
    malefic_transport::SessionConfig {
        ...
        reset_per_message: is_request_response,
        empty_read_is_idle: is_request_response,
    }
}
...
impl MaleficStub {
    pub fn new(instance_id: [u8; 4], channel: MaleficChannel) -> Self {
        let manager = default_manager().unwrap_or_else(|e| {
            debug!("origin modules refresh failed: {:?}", e);
            MaleficManager::new()
        });
