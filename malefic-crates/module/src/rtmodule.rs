...
macro_rules! register_rt_modules {
    ($($(#[$meta:meta])* $module:ty),+ $(,)?) => {
        static _RT_MODULE_REGISTRY: &[$crate::rtmodule::RtModuleDescriptor] = &[
            $(
                $(#[$meta])*
                $crate::rtmodule::RtModuleDescriptor {
                    name_fn: <$module as $crate::rtmodule::RtModule>::name,
                    constructor: || {
                        Box::new(<$module as $crate::rtmodule::RtModule>::new())
                    },
                },
            )+
        ];
        ...
        pub extern "C" fn rt_module_destroy(handle: *mut $crate::abi::RtModuleHandle) {
            if !handle.is_null() {
                unsafe {
                    let _ = Box::from_raw(
                        handle as *mut Box<dyn $crate::rtmodule::ErasedRtModule>
                    );
                }
            }
        }
