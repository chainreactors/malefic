...
pub struct SessionConfig {
    ...
    /// Reset AES-CTR cipher counter before each message.
    /// Required for HTTP transport where the server resets its cipher per request.
    pub reset_per_message: bool,
    /// Treat a zero-byte read before a frame header as an idle no-data response.
    /// This is only appropriate for request/response transports such as HTTP.
    pub empty_read_is_idle: bool,
}
...
impl Default for SessionConfig {
    fn default() -> Self {
        Self {
            ...
            reset_per_message: false,
            empty_read_is_idle: false,
        }
    }
}
...
impl<T: TransportImpl + 'static> Session<T> {
    ...
    pub async fn read(&mut self) -> Result<Option<SpiteData>, TransportError> {
        if self.config.reset_per_message {
            self.cryptor.reset();
        }
    ...
    pub fn split(self) -> (SessionReader, SessionWriter) {
        ...
        (
            ...
            SessionWriter {
                ...
                reset_per_message: self.config.reset_per_message,
            },
        )
    }
...
async fn read_bytes<R: AsyncRead + Unpin>(
    ...
    while dst.len() < target_len {
        ...
        if n == 0 {
            if idle_if_empty && dst.is_empty() && empty_read_is_idle {
                return Ok(None);
            }
