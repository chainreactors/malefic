...
impl Display for HTTPNotOKError {
    fn fmt(&self, f: &mut Formatter) -> Result {
        match self.code {
            Some(code) => write!(
                f,
                "HTTP proxy server responsed with a non-200 error code: {}. ",
                code
            ),
...
impl Display for Socks5HandshakeError {
    fn fmt(&self, f: &mut Formatter) -> Result {
        write!(
            f,
            "Failed to complete handshake with SOCKS 5 server: {}. ",
            self.socket
        )
    }
}
...
const SOCKS5_REP_ERROR_MESSAGE: [&'static str; 8] = [
...
impl Display for Socks5RequestError {
    fn fmt(&self, f: &mut Formatter) -> Result {
        let reason = if self.error_type >= 1 && self.error_type <= 8 {
        ...
        write!(
            f,
            "SOCKS 5 server {} refused request. Reason: {}. ",
            self.socket, reason
        )
    }
}
