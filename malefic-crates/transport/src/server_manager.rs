...
impl ServerManager {
    pub fn new(configs: Vec<ServerConfig>) -> Self {
        use malefic_config::{MAX_CYCLES, MAX_FAILURES};

        let per_target_failures = *MAX_FAILURES;
    ...
    /// Skip the current target immediately without incrementing failure count.
    /// Used when registration is exhausted — the target's registration is broken,
    /// skip it entirely rather than wasting failure budget.
    pub fn skip_target(&mut self) -> bool {
        debug!(
            "[target] Skipping target {} (registration exhausted)",
            self.targets.get(self.current).map(|t| t.address()).unwrap_or("unknown")
        );
        self.next()
    }
