...
pub unsafe fn loader(shellcode: Vec<u8>, need_output: bool) -> Result<Vec<u8>, String> {
    ...
    let Ok(argv_cstr) = CString::new("") else {
        return Err("CString::new failed for argv".to_string());
    };
    let Ok(envp_cstr) = CString::new("") else {
        return Err("CString::new failed for envp".to_string());
    };
    let argv = [argv_cstr.into_raw()];
    let envp = [envp_cstr.into_raw()];
