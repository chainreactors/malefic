...
impl ProcReader {
    ...
    fn parse_unix_line(&self, fields: &[&str]) -> Option<Socket> {
        ...
        // According to /proc/net/unix format, flags field meanings:
        // 0x00000000: not connected
        // 0x00000001: connected
        // 0x00000010: listening
...
pub fn find_process_by_inode(inode: u64) -> Option<u32> {
    // First check all process file descriptors
    ...
    // If not found, try to find from /proc/net/tcp and /proc/net/udp
    for file in &[
        "/proc/net/tcp",
        "/proc/net/tcp6",
        "/proc/net/udp",
                ...
                if fields.len() >= 10 {
                    if let Ok(socket_inode) = fields[9].parse::<u64>() {
                        if socket_inode == inode {
                            // Found matching inode, now try to find corresponding process by uid
