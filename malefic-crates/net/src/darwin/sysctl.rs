...
use super::socket::{Protocol, ProtocolSocketInfo, Socket, TcpSocketInfo, TcpState, UdpSocketInfo};
use byteorder::{ByteOrder, NetworkEndian};
use libc::{self, AF_INET, AF_INET6, IPPROTO_TCP, IPPROTO_UDP, PF_INET};
...
pub fn list_all_fds_for_pid(pid: PID) -> Result<Vec<ProcFDInfo>, io::Error> {
    ...
    if return_code <= 0 {
        ...
        Ok(fds
            .into_iter()
            .filter_map(|fd| ProcFDInfo::try_from_proc_fdinfo(fd).ok())
            .collect())
    }
}
...
pub fn get_sockets_sysctl(protocol: Protocol) -> Result<Vec<Socket>, io::Error> {
    ...
    let number_of_pids = unsafe { proc_listpids(1, 0, std::ptr::null_mut(), 0) };
    ...
    for pid in pids {
        let fds = match list_all_fds_for_pid(pid) {
            Ok(fds) => fds,
            Err(_) => continue, // Skip if insufficient permissions
        };
