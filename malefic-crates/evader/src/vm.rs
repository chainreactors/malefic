...
/// Get CPUID hypervisor vendor string - Most reliable VM detection method
///
/// This function checks CPUID leaf 1 ECX bit 31 (hypervisor bit)
/// This bit is only set in a real VM environment
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
fn get_hypervisor_vendor() -> Option<String> {
    unsafe {
        ...
        if (cpuid_1.ecx & (1 << 31)) == 0 {
            return None; // No hypervisor, definitely not a VM
        }
        ...
        Some(
            String::from_utf8_lossy(&vendor_bytes)
                .trim_end_matches('\0')
                .to_string(),
        )
    }
...
///
/// This method measures CPUID instruction execution time, which is usually slower in VMs than on physical machines
#[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
fn detect_vm_by_timing() -> Option<Vec<String>> {
    ...
    if avg > 1000 {
        Some(vec![format!(
            "CPU timing anomaly detected: {} cycles (threshold: 1000)",
            avg
        )])
    } else {
        None
...
/// Strict VM detection: Only relies on CPUID hypervisor detection
///
/// This is the most reliable method to avoid false positives on host machines. Only when the CPU
/// is truly running under a hypervisor will the CPUID instruction report the hypervisor's presence.
pub fn detect_vm_framework() -> VmDetectionResult {
    // Step 1: Check if hypervisor exists (most reliable method)
    if let Some(vendor) = get_hypervisor_vendor() {
        let mut features = vec![format!("Hypervisor detected via CPUID: {}", vendor)];

        // Step 2: Determine specific VM framework based on vendor string
        if vendor.contains("VBoxVBoxVBox") {
            ...
            // Detected hypervisor but not a known framework
            features.push(format!("Unknown hypervisor: {}", vendor));
            return VmDetectionResult::detected(VmFramework::Unknown, features);
        }
    }

    // Step 3: If no hypervisor detected, try CPU timing detection as last resort
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
    ...
    // Step 4: No VM features detected
