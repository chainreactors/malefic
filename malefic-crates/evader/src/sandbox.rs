...
use malefic_os_win::reg::{RegistryHive, RegistryKey};
use malefic_process::get_processes;
...
fn detect_hardware_anomalies() -> DetectionResult {
    ...
    .is_err()
    {
        result.add_detection(obfstr!("No USB device usage history found").to_string(), 15);
    }
...
fn detect_sysinfo_anomalies() -> DetectionResult {
    ...
    if let Ok(processes) = get_processes() {
        let suspicious_processes = vec![
            ...
            obfstr!("regmon").to_string(),
            obfstr!("filemon").to_string(),
            obfstr!("procexp").to_string(),
            obfstr!("apimonitor").to_string(),
            // debug check
            obfstr!("idaq").to_string(),
...
fn detect_time_anomalies() -> DetectionResult {
    ...
    if sleep_ratio < 0.2 {
        // Only flag if < 20ms for 100ms request
        result.add_detection(
            format!(
                "{}: {:.2}",
...
pub fn detect_sandbox() -> SandboxDetection {
    ...
    malefic_common::debug!("Hardware detection: {:?}", detection.hardware_detection);
    malefic_common::debug!("System info detection: {:?}", detection.sysinfo_detection);
    malefic_common::debug!("Time detection: {:?}", detection.time_detection);
    ...
    malefic_common::debug!("sandbox confidence score: {}", detection.confidence_score);
    detection
}
