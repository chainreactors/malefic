...
fn require_fn<T: Copy>(func: Option<T>, name: &str) -> Result<T, String> {
    func.ok_or_else(|| format!("REM function {} not initialized", name))
}
...
pub fn rem_dial(cmdline: &str) -> Result<String, String> {
    ...
    unsafe {
        ...
        let dial_fn = require_fn(funcs.rem_dial, "rem_dial")?;
        let result = dial_fn(c_cmdline.as_ptr());
        if result.err != 0 {
...
pub fn memory_dial(memhandle: &str, dst: &str) -> Result<i32, String> {
    ...
    unsafe {
        ...
        let dial_fn = require_fn(funcs.memory_dial, "memory_dial")?;
        let result = dial_fn(c_memhandle.as_ptr(), c_dst.as_ptr());
        if result.err != 0 {
            Err(handle_rem_error(result.err))
        } else {
            Ok(result.val)
...
pub fn memory_read(handle: i32, buf: &mut [u8]) -> Result<usize, String> {
    let funcs = get_functions()?;
    unsafe {
        let read_fn = require_fn(funcs.memory_read, "memory_read")?;
        let result = read_fn(
            handle,
...
pub fn memory_write(handle: i32, buf: &[u8]) -> Result<usize, String> {
    let funcs = get_functions()?;
    unsafe {
        let write_fn = require_fn(funcs.memory_write, "memory_write")?;
        let result = write_fn(
            handle,
...
pub fn memory_close(handle: i32) -> Result<(), String> {
    let funcs = get_functions()?;
    unsafe {
        let close_fn = require_fn(funcs.memory_close, "memory_close")?;
        let err_code = close_fn(handle);
        if err_code != 0 {
