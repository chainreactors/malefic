...
pub struct RtVTable {
    ...
    /// Set to false when the DLL is about to be unloaded.
    /// All callers must check this before invoking any function pointer.
...
impl RtBundle {
    ...
    pub fn vtable(&self) -> &Arc<RtVTable> {
        &self.vtable
    }
...
impl ModuleImpl for RtModuleProxy {
    async fn run(
        ...
        // Check if the DLL is still alive before calling into it.
        if !self.vtable.alive.load(Ordering::Acquire) {
            return Err(anyhow::anyhow!(
                "module '{}': plugin already unloaded",
                self.name
            ));
        }
        ...
        let input_task = malefic_common::spawn(async move {
            while !input_shutdown_clone.load(Ordering::Relaxed) {
        ...
        // Signal input forwarder to stop, then abort and await it to ensure
        // it no longer references recv_channel before we return.
        input_shutdown.store(true, Ordering::Relaxed);
        #[cfg(feature = "tokio")]
        {
            input_task.abort();
            let _ = malefic_common::join_handle(input_task).await;
        }
        #[cfg(feature = "async-std")]
        {
            let _ = input_task.cancel().await;
        }
        #[cfg(feature = "smol")]
        {
            let _ = input_task.cancel().await;
        ...
        // Destroy the module instance (only if DLL is still alive).
        let vtable_alive = Arc::clone(&self.vtable);
        if vtable_alive.alive.load(Ordering::Acquire) {
            unsafe {
...
pub struct LoadedPlugin {
    ...
    module_names: Vec<String>,
    /// Shared vtable reference — used to signal `alive = false` on unload.
    vtable: Arc<RtVTable>,
}
...
impl PluginLoader {
    ...
    pub unsafe fn load(
        ...
        let vtable_ref = Arc::clone(&rt_bundle.vtable);
    ...
    pub unsafe fn unload(&mut self, name: &str) -> Option<Vec<String>> {
        if let Some(plugin) = self.plugins.remove(name) {
            ...
            // Mark vtable as dead — any in-flight tasks holding Arc<RtVTable>
            // will see alive=false and bail out instead of calling freed memory.
            plugin.vtable.alive.store(false, Ordering::Release);
