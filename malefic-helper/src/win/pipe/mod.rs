...
use windows::Win32::Foundation::{HANDLE, INVALID_HANDLE_VALUE, GetLastError, GENERIC_READ, GENERIC_WRITE, ERROR_PIPE_BUSY,ERROR_ACCESS_DENIED, FALSE, CloseHandle, WIN32_ERROR, ERROR_PIPE_CONNECTED};
use windows::Win32::Storage::FileSystem::{ ReadFile, WriteFile, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, CreateFileW, FILE_WRITE_ATTRIBUTES, FILE_READ_ATTRIBUTES, FILE_SHARE_READ, FILE_SHARE_WRITE, FlushFileBuffers, FILE_FLAG_FIRST_PIPE_INSTANCE, PIPE_ACCESS_DUPLEX};
use windows::Win32::System::Pipes::{ ConnectNamedPipe, DisconnectNamedPipe, NAMED_PIPE_MODE, WaitNamedPipeW, SetNamedPipeHandleState, PIPE_READMODE_BYTE, PIPE_TYPE_BYTE,PIPE_WAIT, CreateNamedPipeW, PIPE_TYPE_MESSAGE};
            ...
            match err {
                ...
                ERROR_ACCESS_DENIED => {
                    if mode == (GENERIC_READ.0 | GENERIC_WRITE.0) {
                        return PipeClient::create_file(name, GENERIC_READ.0 | FILE_WRITE_ATTRIBUTES.0);
                    }
                    if mode == (GENERIC_READ.0 | FILE_WRITE_ATTRIBUTES.0) {
                        return PipeClient::create_file(name, GENERIC_WRITE.0 | FILE_READ_ATTRIBUTES.0);
                    }
    ...
    pub fn connect_ms(name: &str,timeout: u32) -> io::Result<PipeClient> {
        ...
        loop {
            match PipeClient::create_file(&wide_pipe_name, GENERIC_READ.0 | GENERIC_WRITE.0) {
                Ok(handle) => {
                    let result = unsafe {
                        let mode = NAMED_PIPE_MODE(PIPE_TYPE_BYTE.0 | PIPE_READMODE_BYTE.0 | PIPE_WAIT.0);
                ...
                Err(err) => {
                    if err.raw_os_error().unwrap() == ERROR_PIPE_BUSY.0 as i32 {
                        if !waited {
                            waited = true;
                            let result = unsafe { WaitNamedPipeW(PCWSTR(wide_pipe_name.as_ptr()), timeout) };
                            if result == FALSE {
                                return Err(err);
                            }
                        } else {
