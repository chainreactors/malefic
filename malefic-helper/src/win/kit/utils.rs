...
pub fn get_sid() -> Option<String> {
    let name = get_user_name_w();
    if name.is_none() {
        return None;
    }
    let name = name.unwrap();
    return look_up_account_name(name);
}

pub fn get_gid() -> Option<String> {
    let name = get_computer_name_ex_w(ComputerNameDnsHostname);
    if name.is_none() {
        return None;
    }
    let name = name.unwrap();
    return net_local_group_get_members(name);
}
